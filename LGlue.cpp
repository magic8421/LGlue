...
void Lua::load(const std::string& name, int i) {
    ...
    if (lua_gettop(vm) - (i>0?i:-i) < 0 || !lua_istable(vm, i)) {
		if (-1 == i) {
			//t = LUA_GLOBALSINDEX;
			int type = lua_rawgeti(vm, LUA_REGISTRYINDEX, LUA_RIDX_GLOBALS); // global
			lua_getfield(vm, -1, name.c_str()); // global result
			lua_remove(vm, -2); // result
		}
		else {
...
void Lua::save(const std::string& name, const int i) {
    ...
    if (lua_gettop(vm) - (i>0?i:-i) < 0 || !lua_istable(vm, i)) {
		if (-2 == i) {
			//t = LUA_GLOBALSINDEX; // value
			lua_rawgeti(vm, LUA_REGISTRYINDEX, LUA_RIDX_GLOBALS); // value global
			lua_pushvalue(vm, -2); // value global value
			lua_setfield(vm, -2, name.c_str()); // value global
			lua_pop(vm, 2);
		}
		else {
